...
public class ReturnHomeCommand {
    public ReturnHomeCommand(CommandDispatcher<CommandSource> dispatcher) {
        dispatcher.register(Commands.literal("base").then(Commands.literal("return").executes((command) -> {
    ...
    private int returnHome(CommandSource source) throws CommandSyntaxException {
        ...
        boolean hasHomepos = player.getPersistentData().getIntArray(Mttm.MOD_ID + "homepos").length != 0;

        if(hasHomepos) {
            int[] playerPos = player.getPersistentData().getIntArray(Mttm.MOD_ID + "homepos");
